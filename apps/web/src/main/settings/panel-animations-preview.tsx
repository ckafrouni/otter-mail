...
/** A tiny window whose side panels open and close at the chosen speed (ported from Otter Code). */
export function PanelAnimationsPreview({ durationMs }: { durationMs: number }) {
  ...
  return (
    <button
      ...
      className="flex h-10 w-full cursor-pointer overflow-hidden rounded-lg border border-border bg-canvas p-1 shadow-xs/5 outline-none focus-visible:ring-2 focus-visible:ring-focus-ring focus-visible:ring-offset-1 focus-visible:ring-offset-canvas"
    ...
    >
      <span
        aria-hidden
        className={cn(
          "h-full shrink-0 overflow-hidden rounded-md bg-sidebar-surface",
          transitionClass,
          panelsOpen ? "w-4" : "w-0",
        )}
      />
