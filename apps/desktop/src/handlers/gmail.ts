...
export function registerGmailHandlers(): void {
  ...
  ipcMain.handle("gmail:listAccounts", async (_event) => {
    ...
    try {
      ...
      return accounts.map((account) =>
        isSignedIn(account.id) ? account : { ...account, signedOut: true },
      );
    } catch (err) {
  ...
  ipcMain.handle("gmail:addAccount", async (_event, params: unknown) => {
  ...
  ipcMain.handle("gmail:cancelAddAccount", async () => {
    cancelSignIn();
  });
    ...
    try {
      ...
      return await runAsTask(asString(p?.taskId), async () => {
        ...
        event.sender.startDrag({
          file: filePath,
          icon: icon.isEmpty() ? await app.getFileIcon(filePath, { size: "normal" }) : icon,
        });
        return { ok: true };
      });
    } catch (err) {
